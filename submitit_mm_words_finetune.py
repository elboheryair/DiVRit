...
from configs.config_maps import MODEL_PROTOTYPE_CONFIGS, TRAINING_CONFIGS, MM_WORDS_CONFIGS
...
class SubmititTrainingArguments:
    ...
    finetuning_config_name: Optional[str] = field(default=None, metadata={"help": "Name of the training config to use"})
...
class Trainer:
    ...
    def __call__(self):
        import scripts.training.run_MM_words_diacritizer as trainer
...
def get_config_dict(**kwargs):
    ...
    # Training config
    finetuning_config_name = kwargs.pop("finetuning_config_name")
    if finetuning_config_name:
        training_config = load_json(MM_WORDS_CONFIGS[finetuning_config_name])
...
def main():
    ...
    # # my addition, potentially make a job terminate when CUDA error occure
    ...
    additional_parameters_dict = {
        ...
        # 'gpus': partition + ":" + str(num_gpus_per_node),
        # 'time': "7-00:00:00",
        'time': "14-00:00:00",
    }
    ...
    executor.update_parameters(
        ...
        mem_gb=24 * num_gpus_per_node,
        gpus_per_node=num_gpus_per_node,
        slurm_additional_parameters=additional_parameters_dict,
    ...
    if partition != "main":
        executor.update_parameters(slurm_qos="pintery")
    ...
    # print the job tracking command for the terminal
    print(f"scontrol show job {job.job_id}")
...
'''
python submitit_mm_words_finetune.py \
	--job_dir diacritics/diacritizing/mm_words_finetuning/base/ \
	--prototype_config_name scratch_noto_span0.25-dropout \
	--finetuning_config_name base \
	--nodes 1 \
	--ngpus 2 \
	--partition rtx6000
'''
